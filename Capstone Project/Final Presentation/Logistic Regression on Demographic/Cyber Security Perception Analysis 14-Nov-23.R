...
data$predicted_concern <- predict(final_model, type = "response")

# Convert predicted probabilities to binary predictions (0 or 1)
data$predicted_binary <- ifelse(data$predicted_concern >= 0.5, 1, 0)

# Confusion Matrix
conf_matrix <- table(data$predicted_binary, data$concern_binary)
#            Actual 0 Actual 1
# Predicted 0      41       33
# Predicted 1       5       25
# Accuracy
...
# - The number of instances correctly predicted as positive (Concerned).
# 
# - True Negatives (TN): 41
# - The number of instances correctly predicted as negative (Not Concerned).
# 
...
# 
# Formulas:
#   - Accuracy: (TP + TN) / (TP + TN + FP + FN) = (25 + 41) / (25 + 41 + 33 + 5) = 0.61 or 61%
# 
# - Sensitivity (True Positive Rate, Recall): TP / (TP + FN) = 25 / (25 + 5) = 0.83 or 83%
# 
# - Specificity (True Negative Rate): TN / (TN + FP) = 41 / (41 + 33) = 0.55 or 55%
# 
# - F1 Score: 2 * (Precision * Recall) / (Precision + Recall)
# - Precision: TP / (TP + FP) = 25 / (25 + 33) = 0.43 or 43%
# - Recall: TP / (TP + FN) = 25 / (25 + 5) = 0.83 or 83%
# - F1 Score = 2 * (0.43 * 0.83) / (0.43 + 0.83) = 0.67 or 67%
...
# Create ROC curve
roc_curve <- roc(data$concern_binary, data$predicted_concern)
